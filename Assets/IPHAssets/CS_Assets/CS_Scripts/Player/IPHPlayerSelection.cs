...
namespace InfiniteHopper
{
    ...
    public class IPHPlayerSelection : MonoBehaviour
    {
        ...
        void SetPlayer(int playerNumber)
        {
            ...
            if (IPHDataStorage.playerData.tokens >= playerList[playerNumber].tokensToUnlock)
            {
            ...
            else //Otherwise, display the number of tokens needed before this character is unlocked
            {
                ...
                if (tokenIcon)
                {
                    ...
                    tokenIcon.Find("Text").GetComponent<Text>().text = (playerList[playerNumber].tokensToUnlock - IPHDataStorage.playerData.tokens).ToString();
                }
            }
        }
    }
