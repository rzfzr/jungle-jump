...
namespace InfiniteHopper
{
    ...
    public class IPHGameController : MonoBehaviour
    {
        ...
        public IPHPlayerScriptable[] availablePlayers;
        ...
        {            
            ...
            IPHDataStorage.playerData.rounds++;
            ...
            else
            {
                ...
                if (playerObject)
                {
                    if (cameraObject)
                    {
                        ...
                        cameraObject.GetComponent<Rigidbody2D>().velocity = new Vector2((playerObject.position.x - cameraObject.position.x) * cameraSpeed, cameraObject.GetComponent<Rigidbody2D>().velocity.y);
                    }
        ...
        void createColumn(int columnCount, bool giveBonus)
        {
            ...
            while (columnCount > 0)
            {
                ...
                else if (itemRateCount >= itemRate)
                {
                    ...
                    Transform item = Instantiate(items[itemIndex], newColumn.position, Quaternion.identity) as Transform;

                    //Set item to parent column
                    item.SetParent(newColumn);
        ...
        void ChangeScore(Transform landedObject)
        {
            ...
            for (index = 0; index < landingBonuses.Length; index++)
            {
                ...
                if (bonusGiven == false && landingDistance <= landingBonuses[index].landDistance)
                {
                    ...
                    if (index == 0)
                    {
                        ...
                        //Check if currentStreak is greater then longestStreak
                        if (currentStreak > IPHDataStorage.playerData.longestStreak)
                            IPHDataStorage.playerData.longestStreak = currentStreak;
                    }
                    else
                    {
                        currentStreak = 0;
            ...
            //Check if currentDistance is greater then longestDistance
            if (currentDistance > IPHDataStorage.playerData.longestDistance)
                IPHDataStorage.playerData.longestDistance = currentDistance;
        }
        ...
        IEnumerator GameOver(float delay)
        {
            ...
            if (gameOverCanvas)
            {
                ...
                IPHDataStorage.instance.AddNewHightscore(score);

                //Write the high sscore text
                gameOverCanvas.Find("TextHighScore").GetComponent<Text>().text = "HIGH SCORE " + IPHDataStorage.playerData.highScore.ToString();                
            }
        }
        ...
        public void Continue()
        {
            if (continues > 0)
            {
                ...
                playerObject.position = lastLandedObject.position;
        ...
        void SetPlayer(int playerNumber)
        {
            ...
            IPHPlayer player = Instantiate(playerBase, new Vector3(0, 6, -1), Quaternion.identity).GetComponent<IPHPlayer>();
            player.PlayerPresets = availablePlayers[playerNumber];
        ...
        {
            playerObject.SendMessage("StartJump", playerAutoJump);
        }

        //This function sends an end jump command to the current player
        void EndJump()
        {
            playerObject.SendMessage("EndJump");
        }
        ...
        void RescalePlayer(float targetScale)
        {
            if (playerObject)
            {
                playerObject.SendMessage("Rescale", targetScale);
            }
        }
        ...
        IEnumerator ActivatePowerup(int powerupIndex)
        {
            ...
            if (powerups[powerupIndex].duration > 0)
            {
                ...
                //Check if currnet powerups streak is greater then longest powerup streak
                if (totalPowerups > IPHDataStorage.playerData.longestPowerUpStreak)
                    IPHDataStorage.playerData.longestPowerUpStreak = currentPowerUpStreak;
            }
            else //Otherwise, activate the power up functions
            {
                ...
                //Check if total powerups is greater then storage data
                if (totalPowerups > IPHDataStorage.playerData.totalPowerups)
                    IPHDataStorage.playerData.totalPowerups = totalPowerups;
        ...
        /// <summary>
        /// Get the triggered token
        /// </summary>
        /// <param name="tokenWeight"></param>
        /// <returns></returns>
        IEnumerator GetToken(int tokenWeight)
        {
            //Add amount of token to player data.
            IPHDataStorage.playerData.tokens += tokenWeight;
            yield return null;
        }
